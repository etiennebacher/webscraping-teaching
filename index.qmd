...
for (page_index in 1:2348) {
  ...
  message(paste0("Start scraping of page", page_index))
  ...
  for (modal_index in buttons) {
    ...
    message(paste0("  Scraped modal", modal_index))
  }
  
  # Once all modals of a page have been scraped, go to the next page (except
  # if we're on the last page)
  
  message(paste0("Finished scraping of page", page_index))
}
```
...
Example using the package `logger` (there is also `logging`, `futile.logger`,
etc.):
...
![](img/logfile_demo.png){width="500" height="500"}
